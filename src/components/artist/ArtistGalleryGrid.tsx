...
export default function ArtistGalleryGrid({ posts, accentColor, artistProfile }: ArtistGalleryGridProps) {
  ...
  // Reset zoom when changing posts
  useEffect(() => {
    setZoomLevel(1);
  }, [selectedPost]);
      ...
      {selectedPost && (
        ...
        >
          ...
          <div className="relative w-full h-full flex flex-col md:flex-row" onClick={e => e.stopPropagation()}>
            ...
            <div className="flex-1 relative flex items-center justify-center p-4 md:p-8 overflow-hidden bg-black/20">
              ...
              >
                <img
                  ...
                  className="max-w-full max-h-[80vh] md:max-h-[90vh] object-contain shadow-2xl rounded-lg"
              ...
              <button
                onClick={handlePrev}
                className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full
              ...
              >
                <svg className="w-5 h-5 md:w-6 md:h-6 -translate-x-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <button
                onClick={handleNext}
                className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 md:w-12 md:h-12 rounded-full
                           bg-black/50 hover:bg-white/10 text-white/70 hover:text-white
                           flex items-center justify-center backdrop-blur-md border border-white/10
              ...
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex gap-2">
                <button
                  ...
                  className="w-10 h-10 rounded-full bg-black/50 hover:bg-white/10 disabled:opacity-30 
            ...
            {/* Sidebar / Bottom Bar for Details */}
            <div className={`
              w-full md:w-[360px] bg-neutral-900/95 backdrop-blur-md border-l border-white/5
              flex flex-col p-6 z-20 transition-all duration-300
              absolute bottom-0 md:relative md:inset-auto h-auto md:h-full
              rounded-t-[24px] md:rounded-none shadow-[0_-10px_40px_rgba(0,0,0,0.5)] md:shadow-none
            `}>
              ...
              <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-6 mt-6 md:mt-2">
                  ...
                  {selectedPost.featuredWork ? (
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedPost.featuredWork.tags.map((tag) => (
                            <span
                              key={tag}
                              className="px-2 py-1 rounded-md text-[10px] font-medium bg-white/5 text-white/60 border border-white/5"
                            >
                              #{tag}
                            </span>
                          ))}
                    ...
                    <div className="text-center py-12">
                      <p className="text-white/50 text-sm italic">
                        {selectedPost.caption || 'No description available for this work.'}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="pt-6 border-t border-white/5 mt-auto space-y-3">
                <a
                  href="/booking"
                  className="block w-full py-3.5 rounded-xl text-center text-sm font-semibold text-black transition-all hover:scale-[1.02]"
