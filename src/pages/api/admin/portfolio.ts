...
export const POST: APIRoute = async (context) => {
  ...
  try {
    ...
    if (!artist_id || !title || !image_url) {
      return Response.json(
        { success: false, error: 'Missing required fields: artist_id, title, image_url' },
        { status: 400 },
      );
    }
    ...
    return Response.json({
      success: true,
      data: { id, artist_id, title, image_url },
    });
  } catch (err) {
    console.error('Admin portfolio POST error:', err);
    return Response.json(
      { success: false, error: 'Internal server error' },
      { status: 500 },
    );
...
export const PATCH: APIRoute = async (context) => {
  ...
  try {
    ...
    console.error('Admin portfolio PATCH error:', err);
    return Response.json(
      { success: false, error: 'Internal server error' },
      { status: 500 },
    );
